...
if (isset($_SESSION['nome_usuarios'])){
	$cond = "WHERE autor_doc_port= '{$_SESSION['nome_usuarios']}'";
} 
$columns = array(
	0 => 'id_doc_port',
	1 => 'titulo_doc_port',
...
$result_doc = "SELECT * FROM documentos_port $cond";
$resultado_doc = mysqli_query($conn, $result_doc);
$linhas = mysqli_num_rows($resultado_doc);
//pegar dados

//Obter dados
$result_doc_port = "SELECT id_doc_port, titulo_doc_port,data_registro_port,autor_doc_port FROM documentos_port $cond";
...
while ($row_doc_port = mysqli_fetch_array($resultado_doc_port)) {
	...
	$dado[] = databr($row_doc_port["data_registro_port"]);
	...
	if ($cond != ""){
	$dado[] = ' <a href="javascript:confirmar('.$row_doc_port['id_doc_port'].',1) "> <button type="button" class="btn btn-primary">Excluir</button></a>';
	}
	$dados[] = $dado;
}
