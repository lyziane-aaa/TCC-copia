    ...
    <body class="tema-escuro">
        ...
        ?>
            <form action="inserir_emprestimo.php" class="cadastro" method="post">
                ...
                <label for="obj_emp">Objetos disponíveis para empréstimo</label>
                <select name="obj_emp" id="nome-objeto">
                    <?php

                    $resultado_pat = mysqli_query($conn, "SELECT * FROM patrimonioativo WHERE emprestado_pat = 1 && emprestavel = 0") or die("erro " . mysqli_error($conn));
                ...
                <label for="data_emp">Gremista que emprestou:</label>
                <input type="text" name="data_emp" required readonly value ="<?php $data = date('d/m/Y - H:i:s'); echo $data;?>">
